...
public class BastionFragranceBlock extends BaseEntityBlock {
    public static final MapCodec<BastionFragranceBlock> CODEC = simpleCodec(BastionFragranceBlock::new);
    public static final VoxelShape SHAPE = Block.box(2.0D, 0.0D, 2.0D, 14.0D, 10.0D, 14.0D);
    private final String myType = "bastion";

    public BastionFragranceBlock(Properties properties) {
        super(properties);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hit) {
        if (!level.isClientSide) {
            // Your custom check layout goes here (e.g., displaying bounded player text data)
        }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        if (!level.isClientSide && placer instanceof Player player && level instanceof ServerLevel serverLevel) {
            ...
            if (!data.isPlayerBoundToType(player.getUUID(), myType)) {
                ...
                // Hook your binding update logic safely here
            }
        }
    }
    ...
    public BlockEntity newBlockEntity(BlockPos pos, BlockState state) {
        return null; // Return your specific BlockEntity if needed
    }
}