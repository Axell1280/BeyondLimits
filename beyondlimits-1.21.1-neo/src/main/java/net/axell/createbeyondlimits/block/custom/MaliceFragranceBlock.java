...
public class MaliceFragranceBlock extends BaseEntityBlock {
    public static final MapCodec<MaliceFragranceBlock> CODEC = simpleCodec(MaliceFragranceBlock::new);
    public static final VoxelShape SHAPE = Block.box(2.0D, 0.0D, 2.0D, 14.0D, 10.0D, 14.0D);
    private final String myType = "malice";

    public MaliceFragranceBlock(Properties properties) {
        super(properties);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hit) {
        if (!level.isClientSide) {
            // Tooltip or binding messaging actions
        }
